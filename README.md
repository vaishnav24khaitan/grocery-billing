...
- **Bulk billing** (`/bulk`, billing-staff only): a separate mode for wholesale/credit customers. Pick or create a customer, add **free-form line items** (item name, unit, quantity, per-unit price, and an optional **MRP** to show a discount), record how much was **received now**, and create the bill. Each customer keeps a running **ledger**: total billed, total received, and the **outstanding balance** (carried from previous bills, with dates). You can also **record a deposit** any time money is received, so the leftover balance always stays correct. Kept separate from the retail cash **Sales** report.
