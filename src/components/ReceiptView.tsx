...
import { formatCurrency, type BillRecord } from "@/lib/types";
...
export default function ReceiptView({
  ...
  const isBulk = bill.source === "bulk";
  const title = isBulk ? "Bill / Invoice" : "Tax Invoice";
  const dateStr = new Date(bill.date).toLocaleString("en-IN", {
    dateStyle: "medium",
    timeStyle: "short",
  });
  const balance =
    isBulk && bill.paidNow != null ? bill.total - bill.paidNow : null;
  ...
  async function shareBill() {
    ...
    try {
      ...
      const file = new File([blob], `${bill.billNo}.png`, {
        type: "image/png",
      });
      const text = `${SHOP_NAME} — ${title} ${bill.billNo}\nTotal: ${formatCurrency(
        bill.total
      )}`;
          ...
          <tbody>
            {bill.items.map((l, i) => (
              <tr key={i} className="border-b border-gray-100">
                ...
                <td className="py-1.5 text-right">{formatCurrency(l.rate)}</td>
